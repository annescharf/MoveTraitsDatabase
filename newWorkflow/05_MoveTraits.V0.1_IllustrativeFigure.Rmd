...
MoveTrait.v0.1 <- readRDS(paste0(pathTOfolder,"MoveTrait.v0.1_individual.sum_20250305.rds"))

MoveTrait.v0.1$study_id <- as.integer64(MoveTrait.v0.1$study_id)
```

## Quick summary
We collected movement traits from a total of 119 bird and 61 mammal species represented by 7289 individuals (3856 birds, 3433 mammals). 
Data of 5518 individuals were collected from Movebank and 1788 from Tucker et al.
...
```{r}
MoveTrait.v0.1.sp <- readRDS(paste0(pathTOfolder,"MoveTrait.v0.1_species.sum_20250305.rds"))
```
...
MoveTrait.v0.1.sp[MoveTrait.v0.1.sp$common_name %in% "Artic fox", "common_name"]<-"Arctic fox"
```

```{r, fig.showtext = TRUE}
cols <- c("1" = "#228833", "2" = "#4477AA", "3" = "#AA3377")

 tot <- c("White-tailed deer","Pronghorn","African elephant","Mongolian gazelle","Lion","Roe deer","Cheetah", "Coyote", "South american tapir","African forest elephant","Arctic fox","Reindeer/caribou","Nyala","Raccoon","Plains zebra")
 
 below <- c("White-tailed deer","Pronghorn","Red deer","Arctic fox","Nyala","Raccoon","Plains zebra")
 left <- c("African elephant","Mongolian gazelle","Lion","Roe deer","Cheetah", "South american tapir","African forest elephant", "Coyote","Reindeer/caribou")
...
dat.FigA <-
  ...
  #dplyr::mutate(label_above = common_name) |>
  ...
  filter(!is.na(AdultBodyMass_kg)) |> droplevels() |> 
...
Only including species with at least 10 individuals!! 
...
```{r}
MoveTrait.v0.1_spatial <- readRDS(paste0(pathTOfolder,"MoveTrait.v0.1_withinindividual_20250305.rds"))
```

```{r,echo=F}
d24h.raw <- MoveTrait.v0.1_spatial |> 
  unnest(displ.24h) |> 
  dplyr::select("individual_id","species","common_name","class","movement.mode",
                "D24h","x","y","contact_person_name","source")
```
...
d24h.mule.sf <- sf::st_as_sf(mule, coords = c(7:8))
...
For 267 out of 1653 pairwise comparisons, the two individuals in question move significantly different along the HFI gradient. 
