...
```{r load data}
movedata <- readRDS("CODE_DATABASE/DATA/Tucker_Road_Spatial.rds")
```
...
attr(movedata$TimestampUTC, "tzone") <- "UTC"
```
...
animlocs.1hourly <- movedata %>% 
  ...
  map(~ make_and_resample_track(., sampling.interval = 1, 
                                 tolerance = 15)) %>% 
  setNames(unique(movedata$t_ani_id)) %>% 
  do.call("rbind", .) 
...
data_resampled <- movedata %>%
  ...
  left_join(., movedata[!duplicated(movedata$ID),c("Species","ID","BodyMass_kg","ContactPerson")], 
            by = c("ID" = "ID")) %>% 
  # 1 hourly
...
Dmax24$OrdinalDay <- lubridate::yday(as.Date(Dmax24$YMD))
...
FMaxDispl24h<-function(x){
  ...
  # 24hr Displacement
  Dmax24h.mean<-as.numeric(with(x, tapply(x$Dmax24+0.001,ID, mean)))
  Dmax24h.median<-as.numeric(with(x, tapply(x$Dmax24+0.001,ID, median)))
...
FSumIOU1m<-function(x){
  ...
  # Get sample size per individual
  n.IOU1m.month<-as.numeric(with(x, tapply(x$month,ID, length)))
...
movedata2 <- movedata %>% 
  ...
  dplyr::select(ID,Species,BodyMass_kg,ContactPerson,mean.longitude,mean.latitude) %>% 
  distinct()

DBMoveTrait.v0 <- merge(movedata2, MoveTrait.v0, by = "ID", all.y=T)
DBMoveTrait.v0 <- DBMoveTrait.v0 %>% droplevels()
write.csv(DBMoveTrait.v0,"CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTraitsDB.v0.1_Tucker.csv",
          row.names=F)
saveRDS(DBMoveTrait.v0,"CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTraitsDB.v0.1_Tucker.rds")
```
...
MoveTrait.v0_spatial <- 
  ...
  tidyr::nest(data = -ID)|> 
  dplyr::select(-data) |>  # just a quick trick to keep things flowing.
  left_join(DBMoveTrait.v0[!duplicated(DBMoveTrait.v0$ID),1:66], 
            by = c("ID" = "ID"))  |>  
  # 1 hourly
  left_join(df.traj1h %>%
              data.frame %>% 
  ...
  # Dmax7d
  left_join(Dmax7d %>%
              dplyr::select("ID","week","year_week","Dmax7d","mean.x", "mean.y") %>% #,"week","year_week",s"mean.x", "mean.y"
              tidyr::nest(data = -ID), 
            by = c("ID" = "ID")) %>% 
  ...
  left_join(mcp.weekly %>%
              dplyr::select("ID","week","year_week","area","mean.x", "mean.y") %>% #,"week","year_week",mean.x", "mean.y"
              tidyr::nest(data = -ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(Mcp.7d = data)  |>  
  # mcp.monthly
  left_join(mcp.monthly %>%
              dplyr::select("ID","month","year_month","area","mean.x", "mean.y") %>%  #,"month","year_month"
              tidyr::nest(data = -ID), 
            by = c("ID" = "ID")) %>% 
  ...
  left_join(df.IoU1m %>%
              ungroup() |>
              dplyr::select("ID","month","year_month","IOU1m","mean.x","mean.y") %>% ## month, year_month
              tidyr::nest(data = -ID), 
            by = c("ID" = "ID")) %>% 
...
saveRDS(MoveTrait.v0_spatial,
        "CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTraitsDB.v0.1_spatial_Tucker.rds")  
```
