...
movedata <- readRDS(paste0(pathTOfolder,"combined.df1.rds"))
...
proj4string(locs1h) <- CRS("+init=epsg:4326")
```
...
colnames(df.traj1h)<- c("ID","TimestampUTC","D1h","Longitude","Latitude")
...
proj4string(locs24h) <- CRS("+init=epsg:4326")
...
colnames(df.traj.24h)<- c("ID","TimestampUTC","D24h","Longitude","Latitude")
...
Dmax7d <- Dmax7d %>% group_by(ID) %>% filter(n() >= 10) %>% ungroup() %>% 
  dplyr::select(ID,week, year_week,Dmax7d)
```
...
at.mcp.daily <- spTransform(dat.mcp.daily,sp::CRS("+proj=moll +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +type=crs"))

mcp.daily <- mcp(dat.mcp.daily, percent = 95) %>% data.frame() %>% 
  mutate(YMD = str_split(id, '[.]', simplify = TRUE)[,2],
         ID = str_split(id, '[.]', simplify = TRUE)[,1]) %>% 
  dplyr::select(ID,YMD,area)
...
mcp.weekly <- mcp(dat.mcp.weekly, percent = 95, unout = c( "km2")) %>% data.frame() %>% 
  mutate(week = as.numeric(stringr::str_extract(id, "(\\d+$)")),
         year_week = stringr::str_extract(id, "[^.]*$"),
         ID = str_extract(id, "[^.]+")) %>% 
  dplyr::select(ID,week,year_week,area)
```
...
dat.mcp.monthly <- dat.mcp.monthly %>% filter(!is.na(x_)) %>% filter(!is.na(y_))
...
mcp.monthly <- mcp(dat.mcp.monthly, percent = 95, unout = c("km2")) %>% data.frame() %>% 
  mutate(month = as.numeric(stringr::str_extract(id, "(\\d+$)")),
         year_month = stringr::str_extract(id, "[^.]*$"),
         ID = str_extract(id, "[^.]+")) %>% 
  dplyr::select(ID,month,year_month,area)
```
...
df.IoU24h <- df.traj1h %>% 
  data.frame %>% 
  mutate(ID_Day = paste(ID,YMD,sep=".")) %>% group_by(ID_Day) %>% mutate(cumsumD1h = sum(D1h)) %>% 
  dplyr::select(ID_Day,YMD,ID,cumsumD1h) %>% distinct() %>%
...
df.IoU1m <- df.traj.24h %>% 
  ...
  dplyr::select(ID_Month,Month,year_month,ID,cumsumD24h) %>% distinct() %>%
...
diurn.ind <- df.traj1h |> 
  mutate(lat=Latitude,lon=Longitude,date=as.Date(TimestampUTC)) |> 
...
FDispl1h<-function(x){
  ...
  n1h<-as.numeric(with(x, tapply(x$TimestampUTC,ID, length)))
...
Displ1h <- FDispl1h(data.frame(df.traj1h))
...
FDispl24h<-function(x){
  ...
  # Get sample size per individual
  n24h.days<-as.numeric(with(x, tapply(x$TimestampUTC,ID, length)))
...
Displ24h <- FDispl24h(df.traj.24h)
```
...
MoveTrait.v0 <- full_join(Displ1h, full_join(Displ24h, full_join(MaxDispl24h, 
             ...
             full_join(MCP1m, full_join(IOU24h, full_join(IOU1m, AvgDI)))))))))
...
DBMoveTrait.v02 <- 
movedata2 %>% 
  left_join(DBMoveTrait.v02, by = "ID") %>% 
  droplevels()
```
...
saveRDS(DBMoveTrait.v02,
        "CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTraitsDB.v0.1_movebankI.rds")
write.csv(DBMoveTrait.v02,
          "CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTraitsDB.v0.1_movebankI.csv",row.names=F)
```
...
MoveTrait.v0_spatial <- 
  ...
  tidyr::nest(-ID) %>% 
  dplyr::select(-data) %>% # just a quick trick to keep things flowing.
  left_join(., DBMoveTrait.v0[!duplicated(DBMoveTrait.v0$ID),1:67], 
            by = c("ID" = "ID")) %>% 
  # 1 hourly
  left_join(.,  df.traj1h %>%
              data.frame %>% 
              dplyr::select("ID","TimestampUTC","D1h") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(Displ.1h = data) %>% 
  # 24 hourly
  left_join(.,  df.traj.24h %>%
              dplyr::select("ID","TimestampUTC","D24h") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(Displ.24h = data) %>% 
  # Dmax24
  left_join(.,  Dmax24 %>%
              dplyr::select("ID","YMD","Dmax24") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  ...
  left_join(.,  df.IoU24h %>%
              dplyr::select("ID","YMD","IOU24h") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(IoU.24h = data) %>% 
  # Intensity of Use 24h
  left_join(.,  df.IoU1m %>%
              dplyr::select("ID","Month","year_month","IOU1m") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(IoU.1m = data) %>% 
# Diurnality Index
  left_join(.,  DI %>%
              dplyr::select("ID","YMD","Diurn") %>% 
              tidyr::nest(-ID), 
            by = c("ID" = "ID")) %>% 
  dplyr::rename(Diurnality = data) 

saveRDS(MoveTrait.v0_spatial,"CODE_DATABASE/Database_Tucker_MovebankV0.1/MoveTrait.v0.1_spatial_movebankI.rds")  
```
